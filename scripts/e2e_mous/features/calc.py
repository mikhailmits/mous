...
def run_ui(pid: int, notes: list[str]) -> None:
    leftover = _leftover()
    posted = _tx_count()
    expect(_keys(pid, "a", command=True), "select entry for math")
    _keys(pid, "delete")
    expect(_keys(pid, "45+34"), "type 45+34")
    expect(wait_ui(pid, "= 79", timeout=2.0), "math row 45+34")
    expect(_keys(pid, "return"), "return inserts 79")
    time.sleep(0.1)
    expect(_tx_count() == posted, "math return must not post")
    expect(_same_money(_leftover(), leftover), "math leftover unchanged")
    expect(_keys(pid, "a", command=True), "clear math result")
    _keys(pid, "delete")
    notes.append("math 45+34 → 79")
