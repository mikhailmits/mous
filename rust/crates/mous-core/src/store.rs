...
impl Store {
    ...
    fn init_schema(&self) -> Result<()> {
        self.conn.execute_batch(
            ...
            CREATE TABLE IF NOT EXISTS recurring_good (
                id         INTEGER PRIMARY KEY,
    ...
    fn prune_category_if_empty(&self, category_id: Option<i64>) -> Result<()> {
        if let Some(cid) = category_id {
            ...
            if count == 0 {
                self.conn
                    .execute("DELETE FROM category WHERE id = ?1", params![cid])?;
            }
        }
        Ok(())
    }
    ...
    fn replace_recurring(&self, good_id: i64, schedule: &str) -> Result<()> {
        self.conn.execute(
            "DELETE FROM recurring_good WHERE good_id = ?1",
            params![good_id],
        )?;
        let trimmed = schedule.trim();
        if !trimmed.is_empty() {
            self.conn.execute(
                "INSERT INTO recurring_good (good_id, cron_stamp) VALUES (?1, ?2)",
    ...
    pub fn cur_delete(&self, id: i64) -> Result<()> {
        ...
        // Keep a default currency if any remain.
        if existing.is_default {
            let remaining: Option<i64> = self
                .conn
                .query_row("SELECT id FROM currency ORDER BY id ASC LIMIT 1", [], |r| {
                    r.get(0)
                })
                .optional()?;
    ...
    pub fn tx_new(&self, tx: &NewTx) -> Result<TxView> {
        ...
        let occurred_on = fmt_date(today());
        self.conn.execute(
...
impl RawGood {
    fn into_view(self, default_symbol: &str) -> TxView {
        let (amount, convertible) = match fx::convert(self.value, &self.currency, default_symbol) {
            Some(a) => (a, true),
        ...
        TxView {
            ...
            amount,
            display_currency: default_symbol.to_string(),
            convertible,
