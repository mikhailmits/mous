...
pub struct TxFilter {
    ...
    /// Restrict to this category name (exact, after trim).
...
pub struct NewTx {
    ...
    /// Civil day `YYYY-MM-DD`. `None` means the local calendar day.
...
pub struct TxView {
    ...
    /// UTC midnight of `occurred_on`, matching the old HTTP `occurred_unix_time`.
    pub occurred_unix_time: i64,
    /// User's default currency. Differs from `display_currency` when FX is missing.
